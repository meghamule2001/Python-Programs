...
def main():
    ...
    newdata=list(filter(ChkEven,arr))   #newdata = Filter(arr)
    print("After Filtering data is : ",newdata)

    newdata1=list(map(Increment,newdata))  #newdata1 =Map(newdata)
    print("After map data is : ",newdata1)

    output = functools.reduce(Add,newdata1)   #output =Reduce(newdata1)
